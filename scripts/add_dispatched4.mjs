...
const VIDEOS_DIR = path.join(__dirname, '../public/Dispatched Part 4');
...
async function main() {
  ...
  console.log(`Found ${mp4Files.length} video files in Dispatched Part 4.`);
  ...
  for (let i = 0; i < mp4Files.length; i++) {
    ...
    const goodsId = `dispatched_video_part4_${timestamp}_${i}`;
    ...
    const videoPathWeb = `/Dispatched Part 4/${encodeURIComponent(file)}`;
    const jpgPathWeb = `/Dispatched Part 4/${encodeURIComponent(file.replace(/\.mp4$/i, '.jpg'))}`;
    ...
    const title = `HypeAfnan Dispatch Part 4 #${i + 1}`;

    const indexProduct = {
      id: goodsId,
      goodsId: goodsId,
      title: title,
      searchCode: `DISP_P4_${String(i + 1).padStart(3, '0')}`,
