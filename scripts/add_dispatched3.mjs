...
const VIDEOS_DIR = path.join(__dirname, '../public/Dispatched Part 3');
...
async function main() {
  ...
  console.log(`Found ${mp4Files.length} video files in Dispatched Part 3.`);
  ...
  for (let i = 0; i < mp4Files.length; i++) {
    ...
    const goodsId = `dispatched_video_part3_${timestamp}_${i}`;
    ...
    const videoPathWeb = `/Dispatched Part 3/${encodeURIComponent(file)}`;
    const jpgPathWeb = `/Dispatched Part 3/${encodeURIComponent(file.replace(/\.mp4$/i, '.jpg'))}`;
    ...
    const title = `HypeAfnan Dispatch Part 3 #${i + 1}`;

    const indexProduct = {
      id: goodsId,
      goodsId: goodsId,
      title: title,
      searchCode: `DISP_P3_${String(i + 1).padStart(3, '0')}`,
